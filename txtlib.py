...
class Text:
    ...
    def load_font(self, font, size):
        if font in pygame.font.get_fonts():
            ...
            if font in self.fonts: #self.fonts.has_key(font):
    ...
    def check_fonts(self):
        ...
        for point in self.points:
            ...
            if not font[len(font)-1] in self.fonts: #self.fonts.has_key(font[len(font)-1]):
                self.load_font(font[len(font)-1], size[len(size)-1])
            
            if not size[len(size)-1] in self.fonts[font[len(font)-1]]: #self.fonts[font[len(font)-1]].has_key(size[len(size)-1]):
                self.load_font(font[len(font)-1], size[len(size)-1])
    ...
    # redraw the text
    def update(self, bgcolor=(0,0,0)):
        # control if the needed fonts are loaded
        self.check_fonts()
        
        self.area.fill(bgcolor)
